...
if(isset($_POST['submit']))
{
    ...
    // Create a new folder for the product images using the product ID
    $productFolder = "productimages/" . $productId;
    if (!file_exists($productFolder)) {
        mkdir($productFolder, 0777, true);
    }

    // Move uploaded files to the new folder
    move_uploaded_file($_FILES["productimage1"]["tmp_name"], $productFolder . "/" . $imgnewfile1);
    move_uploaded_file($_FILES["productimage2"]["tmp_name"], $productFolder . "/" . $imgnewfile2);
    move_uploaded_file($_FILES["productimage3"]["tmp_name"], $productFolder . "/" . $imgnewfile3);
...
<html lang="en">
    <head>
        ...
        <title>JR Tech | Add Products</title>
